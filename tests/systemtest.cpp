...
TEST(SysFull, Constructor_InitialState)
{
    ...
    SysTest st;
    auto users = st.adminViewAllUsers();
...
// registerNewPatient: verify ID increments correctly
TEST(SysFull, RegisterPatient_IdIncrement)
{
    ...
    int id1 = -1, id2 = -1;
    for (auto *u : users) {
        if (u->get_email() == "p@mail.com") id1 = u->get_id();
...
// registerNewDoctor: multiple doctors with different specialties
